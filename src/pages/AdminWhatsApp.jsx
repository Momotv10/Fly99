...
import { MessageSquare, Plus, Info, ExternalLink, FileText, History, Settings, TestTube } from 'lucide-react';
...
import WAHAConnectionTester from '@/components/whatsapp/WAHAConnectionTester';
  ...
  const handleSave = async (formData) => {
    try {
      const saveData = {
        ...formData,
        error_message: null,
        messages_sent: formData.messages_sent || 0,
        messages_received: formData.messages_received || 0
      };

      if (editingGateway) {
        await base44.entities.WhatsAppGateway.update(editingGateway.id, saveData);
        if (saveData.status === 'connected') {
          toast.success('✅ تم تحديث البوابة - جاهزة للإرسال مباشرة!');
        } else {
          toast.success('✅ تم تحديث البوابة');
        }
      } else {
        await base44.entities.WhatsAppGateway.create(saveData);
        if (saveData.status === 'connected') {
          toast.success('✅ تم إنشاء البوابة - جاهزة للإرسال مباشرة!');
        } else {
              ...
              <CardContent className="space-y-6">
                <div className="p-4 bg-green-50 rounded-lg">
                  ...
                  <ol className="list-decimal mr-5 space-y-2 text-sm">
                    <li><strong>إنشاء بوابة جديدة:</strong> اضغط على "إضافة بوابة جديدة"</li>
                    <li><strong>أدخل المعلومات:</strong> حدد الاسم، النوع، رابط خادم WAHA، ومفتاح API</li>
                    <li><strong>اختبر الاتصال:</strong> اضغط "اختبار الاتصال" للتأكد من صحة البيانات</li>
                    <li><strong>احفظ البوابة:</strong> بعد نجاح الاختبار يمكنك الحفظ</li>
                    <li><strong>اتصل بالواتساب:</strong> اضغط "اتصال" وامسح QR من هاتفك</li>
                  </ol>
                </div>
